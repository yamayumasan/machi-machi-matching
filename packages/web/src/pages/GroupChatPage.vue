  ...
  <div class="h-screen flex flex-col bg-gray-100">
    ...
    <header class="bg-white shadow-sm flex-shrink-0">
      <div class="container mx-auto px-4 py-3 flex items-center gap-4">
        ...
        <div class="flex-1 min-w-0" @click="showGroupInfo = true">
          <h1 class="font-bold truncate">{{ group?.name || 'グループチャット' }}</h1>
          <p class="text-xs text-gray-500">{{ group?.members.length || 0 }}人のメンバー</p>
        </div>

        <button @click="showGroupInfo = true" class="text-gray-500 hover:text-gray-700">
          <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    >
      ...
      <div v-else-if="messages.length === 0" class="text-center py-8">
        ...
        <p class="text-sm text-gray-400 mt-1">最初のメッセージを送ってみましょう！</p>
      </div>
        ...
        <template v-for="(message, index) in messages" :key="message.id">
          ...
          >
            ...
            <div :class="['max-w-[70%]', message.isOwn ? 'items-end' : 'items-start']">
              <p v-if="!message.isOwn" class="text-xs text-gray-500 mb-1 ml-1">
      ...
      <!-- Typing indicator -->
      <div v-if="typingUsers.length > 0" class="flex items-center gap-2 text-sm text-gray-500">
    ...
    <!-- Input -->
    <div class="bg-white border-t flex-shrink-0 p-3">
        ...
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md max-h-[80vh] overflow-y-auto">
          <div class="p-6">
            <div class="flex items-center justify-between mb-4">
              <h3 class="text-lg font-bold">グループ情報</h3>
              <button @click="showGroupInfo = false" class="text-gray-500 hover:text-gray-700">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="mb-6">
              <div class="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p class="text-sm text-gray-500">{{ group.recruitment.category.name }}</p>
                </div>
              </div>

              <div v-if="group.recruitment.datetime || group.recruitment.datetimeFlex" class="text-sm text-gray-600 mb-2">
              ...
              <div v-if="group.recruitment.location" class="text-sm text-gray-600">
            ...
            <!-- Members -->
            <div>
              <h4 class="font-medium text-gray-700 mb-3">メンバー ({{ group.members.length }})</h4>
              <div class="space-y-3">
                <div
                ...
                >
                  ...
                  <div class="flex-1">
                    <p class="font-medium">
                      {{ member.nickname }}
                      <span v-if="member.role === 'OWNER'" class="text-xs text-primary-600 ml-1">(主催)</span>
                    </p>
                  </div>
                </div>
              </div>
